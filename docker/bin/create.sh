...
IMAGE="gocpplua/base:`cat $DIR/../dev/version`"
if [ $# -eq 2 ]; then
    IMAGE=$2
fi
...
CONTAINER_IP=`hostname -i | tail -n 1 | awk '{print $(NF)}'`
HOST_IP=`cat /etc/hostinfo | tail -n 1 | awk '{print $(NF)}'`
echo "host: $HOST_IP, container: $CONTAINER_IP"
if [ "$CONTAINER_IP" == "$HOST_IP" ]; then
    echo "begin stop staragent in container to avoid ip conflict"
    sudo /etc/init.d/staragentctl stop
    rm -rf /home/staragent/bin/staragentd
    sudo killall -9 staragentd
else
    echo "skip kill staragent as ip not same"
fi
...
if [ -f /etc/bashrc ]; then
    echo -e "\nPS1=\"\\\\n\\\\e[1;37m[\\\\e[m\\\\e[1;36m$NAME\\\\e[m\\\\e[1;37m][\\\\e[m\\\\e[1;32m\\\\u\\\\e[m\\\\e[1;33m@\\\\e[m\\\\e[1;35m\\\\H\\\\e[m \\\\e[4m\\\`pwd\\\`\\\\e[m\\\\e[1;37m]\\\\e[m\\\\e[1;36m\\\\e[m\\\\n\\\\$\"" >> /etc/bashrc
fi
...
sed -i "s/\$NAME/$NAME/g" $CUR_DIR/after_start.sh
...
if ! mount -l | grep /mnt/ram | grep ramfs ; then
    echo "RAMDISK invalid, recreating..."
    sudo mkdir -p "/mnt/ram"
    sudo mount -t ramfs -o size=20g ramfs /mnt/ram
    sudo chmod a+rw /mnt/ram
else
    echo "RAMDISK has already created"
fi
