...
class UTIL_LOCKABLE ThreadMutex {
public:
    ...
    int lock() UTIL_EXCLUSIVE_LOCK_FUNCTION() { return pthread_mutex_lock(&_mutex); }

    int trylock() UTIL_EXCLUSIVE_TRYLOCK_FUNCTION(false) { return pthread_mutex_trylock(&_mutex); }

    int unlock() UTIL_UNLOCK_FUNCTION() { return pthread_mutex_unlock(&_mutex); }
...
class UTIL_LOCKABLE RecursiveThreadMutex : public ThreadMutex {
public:
...
class UTIL_LOCKABLE ThreadCond : public ThreadMutex {
public:
    ThreadCond() {
...
class UTIL_LOCKABLE ProducerConsumerCond : public ThreadMutex {
public:
...
class UTIL_SCOPED_LOCKABLE ScopedLock {
private:
    ThreadMutex &_lock;
...
public:
    explicit ScopedLock(ThreadMutex &lock) UTIL_EXCLUSIVE_LOCK_FUNCTION(lock) : _lock(lock) {
        int ret = _lock.lock();
        ERRNO_FATAL(ret);
    }

    ~ScopedLock() UTIL_UNLOCK_FUNCTION() {
        int ret = _lock.unlock();
        ERRNO_FATAL(ret);
    }
};

class UTIL_LOCKABLE ReadWriteLock {
private:
...
public:
    ...
    int rdlock() UTIL_SHARED_LOCK_FUNCTION() { return pthread_rwlock_rdlock(&_lock); }

    int wrlock() UTIL_EXCLUSIVE_LOCK_FUNCTION() { return pthread_rwlock_wrlock(&_lock); }

    int tryrdlock() UTIL_SHARED_TRYLOCK_FUNCTION(false) { return pthread_rwlock_tryrdlock(&_lock); }

    int trywrlock() UTIL_EXCLUSIVE_TRYLOCK_FUNCTION(false) { return pthread_rwlock_trywrlock(&_lock); }

    int unlock() UTIL_UNLOCK_FUNCTION() { return pthread_rwlock_unlock(&_lock); }

protected:
    pthread_rwlock_t _lock;
};

class UTIL_SCOPED_LOCKABLE ScopedReadLock {
private:
    ReadWriteLock &_lock;
...
public:
    explicit ScopedReadLock(ReadWriteLock &lock) UTIL_SHARED_LOCK_FUNCTION(lock) : _lock(lock) {
        int ret = _lock.rdlock();
        ERRNO_FATAL(ret);
    }
    ~ScopedReadLock() UTIL_UNLOCK_FUNCTION() {
        int ret = _lock.unlock();
        ERRNO_FATAL(ret);
    }
};

class UTIL_SCOPED_LOCKABLE ScopedWriteLock {
private:
    ReadWriteLock &_lock;
...
public:
    explicit ScopedWriteLock(ReadWriteLock &lock) UTIL_EXCLUSIVE_LOCK_FUNCTION(lock) : _lock(lock) {
        int ret = _lock.wrlock();
        ERRNO_FATAL(ret);
    }
    ~ScopedWriteLock() UTIL_UNLOCK_FUNCTION() {
        int ret = _lock.unlock();
        ERRNO_FATAL(ret);
    }
};

class UTIL_SCOPED_LOCKABLE ScopedReadWriteLock {
private:
    ReadWriteLock &_lock;
    char _mode;
...
public:
    ...
    explicit ScopedReadWriteLock(ReadWriteLock &lock, const char mode) UTIL_EXCLUSIVE_LOCK_FUNCTION(lock)
    ...
    ~ScopedReadWriteLock() UTIL_UNLOCK_FUNCTION() {
...
class UTIL_LOCKABLE SpinLock {
public:
    void lock() UTIL_EXCLUSIVE_LOCK_FUNCTION() {
        while (flag.test_and_set(std::memory_order_acquire))
            ;
    }
    void unlock() UTIL_UNLOCK_FUNCTION() { flag.clear(std::memory_order_release); }

private:
    std::atomic_flag flag = ATOMIC_FLAG_INIT;
};

class UTIL_SCOPED_LOCKABLE ScopedSpinLock {
public:
    ScopedSpinLock(SpinLock &lock) UTIL_EXCLUSIVE_LOCK_FUNCTION(lock) : _lock(lock) { _lock.lock(); }
    ~ScopedSpinLock() UTIL_UNLOCK_FUNCTION() { _lock.unlock(); }
