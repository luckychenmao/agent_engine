...
void ToStream(const Any &a, a_Ostream &os, bool isCompact, const std::string &prefix) {
    ...
    {
        os << prefix << util::legacy::ToString(AnyCast<bool>(a));
    }
    /// different numberic types
#define WRITE_DATA(tp)                                                                                                 \
    else if (type == typeid(tp)) {                                                                                     \
        os << prefix << util::legacy::ToString(util::legacy::AnyCast<tp>(a));                                        \
    }
    WRITE_DATA(double)
    WRITE_DATA(float)
    WRITE_DATA(uint16_t)
...
#define WRITE_DATA_AS(tp, as)                                                                                          \
        ...
        os << prefix << util::legacy::ToString(static_cast<as>(util::legacy::AnyCast<tp>(a)));                       \
    }
...
bool Equal(const Any &lhs, const Any &rhs) {
    ...
    if (IsJsonNull(lhs)) {
        ...
        UTIL_LEGACY_THROW(InvalidOperation, "Equal for Any only supports a json-Any.");
    }
}
