...
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(BadUtf8Format, ExceptionBase);
};

class IndexOutOfRange : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(IndexOutOfRange, ExceptionBase);
};
...
T StringTo(const std::string &str) {
    ...
    UTIL_LEGACY_THROW(ParameterInvalidException, "No valid cast given the string:" + str);
}
...
std::vector<T> StringToVector(const std::string &string, const std::string &delim = ",") {
    if (delim.size() == 0) {
        UTIL_LEGACY_THROW(util::legacy::ParameterInvalidException, "Empty delim");
    }
    if (string.size() == 0) {
        return std::vector<T>();
    }
...
std::pair<K, V> StringToPair(const std::string &str, const std::string &delim = ":") {
    ...
    if (pos == std::string::npos) {
        UTIL_LEGACY_THROW(ParameterInvalidException, "Missing delim expected inside pair");
    }
...
StringToMap(const std::string &string, const std::string &itemDelim = ",", const std::string &keyValueDelim = ":") {
    ...
    for (std::vector<std::string>::const_iterator it = part.begin(); it != part.end(); ++it) {
        if (!result.insert(StringToPair<K, V>(*it, keyValueDelim)).second) {
            UTIL_LEGACY_THROW(ParameterInvalidException, "Duplicate keys in map");
        }
    }
    return result;
}
...
std::string ToString(const T &obj) {
    return util::StringUtil::ToString(obj);
}
...
IntegerType BinaryToInteger(const std::string &s) {
    if (s.empty() || s.size() > sizeof(IntegerType)) {
        UTIL_LEGACY_THROW(ParameterInvalidException, "can't convert empty string to integer");
    }
