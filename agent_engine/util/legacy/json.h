...
JsonNumber &JsonNumber::operator=(const T &data) {
    this->AsString() = util::legacy::ToString(data);
    return *this;
}

template <>
inline JsonNumber &JsonNumber::operator=(const uint8_t &data) {
    this->AsString() = util::legacy::ToString(static_cast<uint32_t>(data));
    return *this;
}

template <>
inline JsonNumber &JsonNumber::operator=(const int8_t &data) {
    this->AsString() = util::legacy::ToString(static_cast<int32_t>(data));
    return *this;
}
...
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(JsonNothingError, JsonParserError);
};
