...
inline void FromRapidValue(std::string &t, RapidValue &value) {
    if (!value.IsString()) {
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect String but get: " + FastToJsonString(value));
    }
    t = value.GetString();
}
...
inline void FromRapidValue(bool &t, RapidValue &value) {
    if (!value.IsBool()) {
        ...
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect bool but get: " + FastToJsonString(value));
    }
    t = value.GetBool();
}
...
inline enable_if_t<std::is_integral<T>::value && std::is_signed<T>::value, void> FromRapidValue(T &t,
                                                                                                RapidValue &value) {
    if (!value.IsInt64()) {
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect int64 but get: " + FastToJsonString(value));
    }
    t = (T)value.GetInt64();
}
...
inline enable_if_t<std::is_integral<T>::value && std::is_unsigned<T>::value, void> FromRapidValue(T &t,
                                                                                                  RapidValue &value) {
    if (!value.IsUint64()) {
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect uint64 but get: " + FastToJsonString(value));
    }
    t = (T)value.GetUint64();
}
...
inline enable_if_t<std::is_enum<T>::value, void> FromRapidValue(T &t, RapidValue &value) {
    if (!value.IsInt()) {
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect int but get: " + FastToJsonString(value));
    }
    t = (T)value.GetInt();
}
...
inline enable_if_t<std::is_floating_point<T>::value, void> FromRapidValue(T &t, RapidValue &value) {
    if (!value.IsNumber()) {
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect number but get: " + FastToJsonString(value));
    }
    t = (T)value.GetDouble();
}
...
#define DESERIALIZE_FROM_ARRAY(container, func, value)                                                                 \
    {                                                                                                                  \
        if (!value.IsArray()) {                                                                                        \
            UTIL_LEGACY_THROW(TypeNotMatchException,                                                                   \
...
inline void FromRapidValue(std::map<T, U, C, A> &t, RapidValue &value) {
    if (!value.IsArray()) {
        UTIL_LEGACY_THROW(TypeNotMatchException, "type not match, expect Array but get: " + FastToJsonString(value));
    }
    t.clear();
    for (auto itr = value.Begin(); itr != value.End(); ++itr) {
...
void FastFromJsonString(T &t, const std::string &str) {
    ...
    if (document.HasParseError()) {
        ...
        UTIL_LEGACY_THROW(WrongFormatJsonException, errStream.str());
    }
