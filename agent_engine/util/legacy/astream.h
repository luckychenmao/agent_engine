...
public:
    ...
    virtual int GetThenPeek() {
        if (get() < 0)
            UTIL_LEGACY_THROW(ExceptionBase, "failed to get in GetThenPeek");
        return peek();
    }
    ...
    void FetchDataFromBuff(uint32_t bufferSize) {
        ...
        if (p != 0) {
            ...
            if (remainInBuff >= mTotal) {
                 ...
                 */
                UTIL_LEGACY_THROW(ExceptionBase, "Bad data requirement since parent has no more data to supply");
            }
        ...
        } else // fill buffer
        {
            ...
            if (mHoldInBuff != canRead) {
                UTIL_LEGACY_THROW(ExceptionBase, "no more to read in LengthedStream");
            }
...
public:
    ...
    virtual int GetThenPeek() {
        if (mLocation != mEnd) {
            ...
            UTIL_LEGACY_THROW(ExceptionBase, "Bad Use of GetThenPeek().");
        }
    }
