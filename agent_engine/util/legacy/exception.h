#pragma once
/** util ExceptionBase
 * All exception classes in Util should inherit ExceptionBase
 */

/**
 * Guildelines for exception handling
 * 1. How to throw
 *    UTIL_LEGACY_THROW(the_exception_to_show, description_string)
 *    e.g. UTIL_LEGACY_THROW(ParameterInvalidException, "Cannot be empty string");
 *    If you throw not using UTIL_LEGACY_THROW, the location of throw will not be recorded.
 *
 * 2. Define an new util exception:
 *    a. SHOULD inherit from util::ExceptionBase or its sub-classes.
 *    b. SHOULD use macro UTIL_LEGACY_DEFINE_EXCEPTION inside public: region of the class definition.
 *
...
#define UTIL_LEGACY_THROW(ExClass, args...)                                                                           \
    do {                                                                                                               \
...
#define UTIL_LEGACY_THROW_CHAIN(ExClass, cause, args...)                                                              \
    do {                                                                                                               \
        ExClass tmp_3d3079a0_61ec_48e6_abd6_8a33b0eb91f0(args);                                                        \
        tmp_3d3079a0_61ec_48e6_abd6_8a33b0eb91f0.Init(__FILE__, __PRETTY_FUNCTION__, __LINE__);                        \
...
#define UTIL_LEGACY_DEFINE_EXCEPTION(ExClass, Base)                                                                   \
...
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(InvalidOperation, ExceptionBase);
};

class RuntimeError : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(RuntimeError, ExceptionBase);
};

class TimeoutError : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(TimeoutError, ExceptionBase);
};

class LogicError : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(LogicError, ExceptionBase);
};

class OverflowError : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(OverflowError, ExceptionBase);
};

class AlreadyExistException : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(AlreadyExistException, ExceptionBase);
};

class NotExistException : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(NotExistException, ExceptionBase);
};

class NotImplementedException : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(NotImplementedException, ExceptionBase);
};

// parameter invalid
class ParameterInvalidException : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(ParameterInvalidException, ExceptionBase);
};

// operation is denied
class AuthenticationFailureException : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(AuthenticationFailureException, ExceptionBase);
};

// base class for all exceptions in storage system
class StorageExceptionBase : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(StorageExceptionBase, ExceptionBase);
};

// when create an exist file
class FileExistException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(FileExistException, StorageExceptionBase);
};

// when open/delete/rename/... an non-exist file
class FileNotExistException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(FileNotExistException, StorageExceptionBase);
};

class DirectoryExistException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(DirectoryExistException, StorageExceptionBase);
};

class DirectoryNotExistException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(DirectoryNotExistException, StorageExceptionBase);
};

class SameNameEntryExistException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(SameNameEntryExistException, StorageExceptionBase);
};

// when append/delete a file that being appended
class FileAppendingException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(FileAppendingException, StorageExceptionBase);
};

// when opening a file that cannot be overwritten
class FileOverwriteException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(FileOverwriteException, StorageExceptionBase);
};

// when append/delete a file that being appended
class PangunNotEnoughChunkserverExcepion : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(PangunNotEnoughChunkserverExcepion, StorageExceptionBase);
};

// when read, data is unavailable due to internal error
class DataUnavailableException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(DataUnavailableException, StorageExceptionBase);
};
...
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(StreamCorruptedException, StorageExceptionBase);
};

// when end of stream comes unexpected
class UnexpectedEndOfStreamException : public StorageExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(UnexpectedEndOfStreamException, StorageExceptionBase);
};

// base class for all exceptions in service
class ServiceExceptionBase : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(ServiceExceptionBase, ExceptionBase);
};

// can't get service connection
class ServiceUnavailableException : public ServiceExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(ServiceUnavailableException, ServiceExceptionBase);
};

// internal server error
class InternalServerErrorException : public ServiceExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(InternalServerErrorException, ServiceExceptionBase);
};

// session expired
class SessionExpireException : public ServiceExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(SessionExpireException, ServiceExceptionBase);
};

// running into unimplemented code
class UnimplementedException : public ExceptionBase {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(UnimplementedException, ExceptionBase);
};
