    ...
    while (currPos < byteCount) {
        if ((str[currPos] & 0x80)) {
            ...
            if ((str[currPos] & 0xE0) == 0xC0) {
                ...
                UTIL_LEGACY_THROW(BadUtf8Format, "");
            }
            int i = 0;
            while (++i < charBytes) {
...
std::string::size_type FindNextCharUtf8(const std::string &str, std::string::size_type pos) {
    ...
    if (currPos > byteCount) {
        UTIL_LEGACY_THROW(IndexOutOfRange, "");
    }
    while (currPos < byteCount) {
...
util::legacy::Utf8StringIterator::Utf8StringIterator(std::string string) : mString(string), currPos(0) {}

std::string util::legacy::Utf8StringIterator::NextChar() {
    if (currPos < mString.size()) {
        if ((mString[currPos] & 0x80)) {
            if ((mString[currPos] & 0xE0) == 0xC0) {
                ...
                UTIL_LEGACY_THROW(ParameterInvalidException, "string with illegal uft8 character");
            }
        } else {
            // 1 bytes
...
std::string util::legacy::Utf8Substr(const std::string &str, size_t pos, size_t length) {
...
#define trueThenThrow(test, str)                                                                                       \
    if (test)                                                                                                          \
        UTIL_LEGACY_THROW(util::legacy::ParameterInvalidException, str);
