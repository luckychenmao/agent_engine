...
uint32_t a_Istream::ReadVarint(uint32_t &udata) {
    ...
    while (bytes < MAX_BYTES) {
        byteRoom = get();
        if (byteRoom < 0)
            UTIL_LEGACY_THROW(StreamCorruptedException, "ReadVarint32");
...
uint32_t a_Istream::ReadVarint(uint64_t &udata) {
    ...
    while (bytes < MAX_BYTES) {
        byteRoom = get();
        if (byteRoom < 0)
            UTIL_LEGACY_THROW(StreamCorruptedException, "ReadVarint64");
        udata |= (byteRoom & 0x7f) << (7 * bytes++);
        if (byteRoom & 0x80)
            return bytes;
    }
...
uint32_t a_IstreamBuffer::ReadVarint(uint32_t &udata) {
    ...
    UTIL_LEGACY_THROW(StreamCorruptedException, "ReadVarint32 In a_IstreamBuffer.");
}
...
uint32_t a_IstreamBuffer::ReadVarint(uint64_t &udata) {
    ...
    UTIL_LEGACY_THROW(StreamCorruptedException, "ReadVarint64 In a_IstreamBuffer.");
}
...
uint32_t a_IstreamLengthed::ReadVarint(uint32_t &udata) {
    ...
    UTIL_LEGACY_THROW(StreamCorruptedException, "ReadVarint32 In a_IstreamLenghted.");
}
...
uint32_t a_IstreamLengthed::ReadVarint(uint64_t &udata) {
    ...
    UTIL_LEGACY_THROW(StreamCorruptedException, "ReadVarint64 In a_IstreamLenghted.");
}
