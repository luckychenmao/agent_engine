...
void FromJson(std::pair<T, U> &s, const Any &a) {
    ...
    FromJson(anyVec, a);
    if (anyVec.size() != 2)
        UTIL_LEGACY_THROW(util::legacy::NotJsonizableException,
...
std::string ToJsonString(const T &t) {
    return util::legacy::json::ToString(ToJson(t));
}

template <typename T>
std::string ToJsonString(const T &t, bool isCompact, const std::string &prefix = "") {
    return util::legacy::json::ToString(ToJson(t), isCompact, prefix);
}
