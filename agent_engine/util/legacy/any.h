...
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(BadAnyCast, ExceptionBase)
};

class BadAnyNumberCast : public BadAnyCast {
public:
    UTIL_LEGACY_DEFINE_EXCEPTION(BadAnyNumberCast, BadAnyCast)

    std::string GetMessage() const override { return "util::legacy::AnyNumberCast: Given is not of number type."; }
};
...
const ValueType &AnyCast(const Any &operand) {
    if (operand.GetType() == typeid(ValueType)) {
        ...
        UTIL_LEGACY_THROW(BadAnyCast,
                           std::string("util::legacy::AnyCast: can't cast from ") + operand.GetType().name() + " to " +
...
#define IF_CAST_RETURN(tp)                                                                                             \
    ...
    IF_CAST_RETURN(int32_t)
        ...
        UTIL_LEGACY_THROW(ParameterInvalidException,
...
template <typename T>
typename std::enable_if<!std::is_arithmetic<T>::value, T>::type AnyNumberCast(const Any &number) {
    UTIL_LEGACY_THROW(ParameterInvalidException, "AnyNumberCast only support number to number");
}
} // namespace legacy
} // namespace util
