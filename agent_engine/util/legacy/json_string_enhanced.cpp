...
using namespace util::legacy::json;
...
inline static void trueThenThrow(bool test, const char *s) {
    if (test)
        UTIL_LEGACY_THROW(JsonParserError, s);
}

inline static void trueThenThrow(bool test, const std::string &s) {
    if (test)
        UTIL_LEGACY_THROW(JsonParserError, s.c_str());
}
} // namespace
...
void ParseJson(const std::string &is, size_t &pos, Any &ret) {
    ...
    UTIL_LEGACY_THROW(JsonNothingError, "");
}
...
void ParseJson(const std::string &is, Any &ret) {
    ...
    SkipSpace(is, pos);
    if (pos < is.size()) {
        UTIL_LEGACY_THROW(JsonParserError, "Expect EOF but see more chars");
    }
}
...
void ToString(const Any &a, std::string &os, bool isCompact, const std::string &prefix) {
    ...
    {
        os.append(prefix);
        os.append(util::legacy::ToString(AnyCast<bool>(a)));
    }
    /// different numberic types
#define WRITE_DATA(tp)                                                                                                 \
    else if (type == typeid(tp)) {                                                                                     \
        os.append(prefix);                                                                                             \
        os.append(util::legacy::ToString(util::legacy::AnyCast<tp>(a)));                                             \
    }
    WRITE_DATA(double)
    WRITE_DATA(float)
    WRITE_DATA(uint16_t)
...
#define WRITE_DATA_AS(tp, as)                                                                                          \
        ...
        os.append(prefix);                                                                                             \
        os.append(util::legacy::ToString(static_cast<as>(util::legacy::AnyCast<tp>(a))));                            \
    }
...
#undef WRITE_DATA_AS
    ...
    {
        UTIL_LEGACY_THROW(util::legacy::ParameterInvalidException,
...
void ParseJsonString(const std::string &is, size_t &pos, std::string &ret) {
    ...
    } catch (const util::legacy::BadQuoteString &e) {
        UTIL_LEGACY_THROW(JsonParserError, std::string("BadQuoteString:") + e.what());
    }
    SkipExact(is, pos, "\"");
}

void ParseJsonNum(const std::string &is, size_t &pos, util::legacy::Any &ret) {
    /*
