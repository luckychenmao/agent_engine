...
#undef FROM_BASIC
    ...
    UTIL_LEGACY_THROW(NotSupportException, "RapidValue to Any not support");
}
...
#undef WRITE_DATA_AS
    ...
    {
        UTIL_LEGACY_THROW(ParameterInvalidException, std::string("See unsupported data in Any:") + type.name());
    }
}
