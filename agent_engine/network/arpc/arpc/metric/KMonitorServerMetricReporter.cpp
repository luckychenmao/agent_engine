...
LOG_SETUP(arpc, KMonitorServerMetricReporter);
...
bool KMonitorServerMetricReporter::init() {
    ...
    if (!kMonitor) {
        _enable = false;
        LOG(WARN, "arpc kmonitor server metric reporter init failed");
        return false;
    }
...
#define CHECK_METRIC_NULLPTR(metric)                                                                                   \
    if (metric == nullptr) {                                                                                           \
        _enable = false;                                                                                               \
        LOG(WARN, "arpc kmonitor server metric reporter init failed, regist metric %s failed", #metric);         \
        return false;                                                                                                  \
    }
...
#undef CHECK_METRIC_NULLPTR
    ...
    LOG(INFO, "arpc kmonitor server metric reporter init done");
    return true;
}
