...
#include "util/log.h"
#include "agent_engine/util/time_utility.h"
...
public:
    void markRequestBegin() { _beginTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    void markRequestDone(arpc::ErrorCode error);
    int64_t totalCostUs() const { return nonZeroSubtract(_doneTimeUs, _beginTimeUs); }

    void markRequestUnpackBegin() { _requestUnpackBeginTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    void markRequestUnpackDone() { _requestUnpackDoneTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    int64_t requestUnpackCostUs() const { return nonZeroSubtract(_requestUnpackDoneTimeUs, _requestUnpackBeginTimeUs); }

    void markRequestPendingBegin() { _requestPendingBeginTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    void markRequestPendingDone() { _requestPendingDoneTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    int64_t requestPendingCostUs() const {
        return nonZeroSubtract(_requestPendingDoneTimeUs, _requestPendingBeginTimeUs);
    }

    void markRequestCallBegin() { _requestCallBeginTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    void markRequestCallDone() { _requestCallDoneTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    int64_t requestCallCostUs() const { return nonZeroSubtract(_requestCallDoneTimeUs, _requestCallBeginTimeUs); }

    void markResponsePackBegin() { _responsePackBeginTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    void markResponsePackDone() { _responsePackDoneTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    int64_t responsePackCostUs() const { return nonZeroSubtract(_responsePackDoneTimeUs, _responsePackBeginTimeUs); }

    void markResponseSendBegin() { _responseSendBeginTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
    void markResponseSendDone() { _responseSendDoneTimeUs = util::TimeUtility::CurrentTimeInMicroSeconds(); }
