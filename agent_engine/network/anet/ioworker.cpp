...
#include "agent_engine/network/anet/epollsocketevent.h"
...
#include "agent_engine/network/anet/thread.h"
...
IoWorker::IoWorker() {
    ...
    _epollWaitTimeoutMs = util::EnvUtil::GetEnv("ANET_EPOLL_WAIT_TIMEOUT", _epollWaitTimeoutMs);
}
...
void IoWorker::setPriorityByEnv() {
    int niceValue = util::EnvUtil::GetEnv("ANET_IO_WORKER_NICE_INC", 0);
    if (niceValue != 0) {
